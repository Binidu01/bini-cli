...
| \`${pm} run deploy\` | Deploy the application to your chosen platform |
...
| \`${pm} run deploy\` | Deploy the Windows application (build + sign + distribute) |
...
| \`${pm} run tauri:icon\` | Generate application icons from \`public/logo.png\` |
| \`${pm} run deploy\` | Deploy the macOS application (build + sign + notarize + distribute) |
...
| \`${pm} run tauri:icon\` | Generate application icons from \`public/logo.png\` |
| \`${pm} run deploy\` | Deploy the Linux application (build + package + distribute) |
...
| \`${pm} run deploy\` | Deploy the Android app (build + sign + distribute to stores) |
...
| \`${pm} run deploy\` | Deploy the iOS app (build + sign + distribute to TestFlight/App Store) |
...
|---|---|---|
| Web | \`${pm} run deploy\` | Builds and deploys to your configured hosting platform (Vercel, Netlify, Cloudflare, etc.) |
| Windows | \`${pm} run deploy\` | Builds, signs with Authenticode, and prepares the Windows installer |
| macOS | \`${pm} run deploy\` | Builds, signs with Developer ID, notarizes with Apple, and prepares the DMG |
| Linux | \`${pm} run deploy\` | Builds the AppImage/DEB/RPM and prepares for distribution |
| Android | \`${pm} run deploy\` | Builds a signed APK/AAB and prepares for Play Store distribution |
| iOS | \`${pm} run deploy\` | Builds, signs, and prepares for TestFlight/App Store distribution |
